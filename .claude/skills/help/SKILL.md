...
argument-hint: "optional: your question"
user-invocable: true
---
...
> Feature [tab/slug] is ready for architecture design.
> Run `/architecture features/<tab>/<slug>.md` to create the technical design.

**If features have Tech Design but no implementation:**
> Feature [tab/slug] has a tech design and is ready for implementation.
> Run `/frontend features/<tab>/<slug>.md` to build the UI.
> (If backend is needed, run `/backend` after frontend is done)

**If features are implemented but no QA:**
> Feature [tab/slug] is implemented and ready for testing.
> Run `/qa features/<tab>/<slug>.md` to test against acceptance criteria.

**If features have passed QA but aren't deployed:**
> Feature [tab/slug] has passed QA and is ready for deployment.
> Run `/deploy features/<tab>/<slug>.md` to deploy to production.
