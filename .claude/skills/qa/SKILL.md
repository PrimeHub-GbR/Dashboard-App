...
3. Check recently implemented features for regression testing: `git log --oneline --grep="feat\|fix" -10`
...
### 4. N8N Workflow Testing (wenn Feature einen N8N-Workflow hat)

Check if the feature spec has a `## N8N Workflow` section. If yes, test:

1. **Workflow active?** Check via MCP `list_workflows` or N8N Dashboard
2. **Happy Path Test**: Call webhook with valid minimal payload
   - Verify: callback received → job status = `completed`
   - Verify: `result_url` exists and file is downloadable
   - Verify: `summary` data is plausible
3. **Error Path Test**: Call webhook with invalid/missing payload
   - Verify: error callback received → job status = `failed`
   - Verify: error message is readable (not just HTTP 500)
4. **N8N Execution Log**: No unexpected errors, all nodes executed
5. **End-to-End Integration**: Full run via Dashboard UI → N8N → Callback → Download
   - Verify: status badge transitions correctly (running → completed/failed)
   - Verify: download button appears and file is correct

Document N8N workflow bugs with: affected node name + N8N execution ID.
...
## Git Commit
```
test(area/slug): Add QA test results for [feature name]
```
Example: `test(repricer/dashboard): Add QA test results`
