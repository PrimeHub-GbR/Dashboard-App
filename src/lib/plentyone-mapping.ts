...
export const MAPPING_SPALTEN: MappingZeile[] = [
  ...
  A('marktplatz_id', 'SKU » Marktplatz-ID', '4 = Amazon (der generische Kanal, wie 2.00 „Ebay“). NICHT 4.01 — damit legt der Import stillschweigend gar keine SKU an, und ohne SKU findet der Amazon-FBA-Bestandsabgleich die Variante nicht: kein Bestand, kein Log-Eintrag, keine Fehlermeldung. Belegt am 07.09.2026. Diese SKU trägt den BESTAND — für den VERSAND braucht es zusätzlich die zweite SKU aus Spalte 60.', 'fest'),
  ...
  { spalte: 'hersteller_id', zielfeld: 'Artikel » Hersteller-ID', herkunft: 'fest', block: 'C',
    beschreibung: 'Die numerische ID des Verlags aus PlentyONE. Nachgeschlagen aus der Liste, die der eBay-Bericht pflegt — vergeben werden die IDs von PlentyONE selbst. Leer, solange der Verlag dort noch nicht angelegt ist; dann erst Hersteller importieren und einmal den Bericht laufen lassen.' },  { spalte: 'marktplatz_id_fba', zielfeld: 'SKU » Marktplatz-ID', herkunft: 'fest', block: 'A',
    beschreibung: 'Immer 104 = „Amazon FBA“ (generisch, NICHT 104.01). Gehört NICHT in den Artikelimport, sondern in einen eigenen, schlanken zweiten Import — PlentyONE kann je Importlauf nur eine SKU-Zeile schreiben. Ohne diese zweite SKU scheitert die Versandfreigabe an FBA mit „sellerSku must be of type string, null given“: der Multi-Channel-Auftrag läuft über die SP-API-Anbindung und sucht die SKU unter 104, nicht unter 4. Belegt am 09.09.2026 an Variante 1207 — nach dem Anlegen der zweiten Zeile nahm Amazon den Auftrag an.' },
]
